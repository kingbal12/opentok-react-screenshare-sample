...
export const calendarfetchEvents = (userid, monthstart, monthend) => {
  return async (dispatch) => {
    await axios
      .get(
        "https://teledoc.hicare.net:446/v1/doctor/appointment/appointments",
        {
          params: {
            user_id: userid,
      ...
      .then((response) => {
        ...
        if (response.data.status === "200") {
          ...
          for (let i = 0; i < length; i++) {
            ...
            jsonObj.id = i + 1;
            jsonObj.title =
              localFormDateSub(response.data.data[i].APPOINT_TIME) +
              " " +
              response.data.data[i].F_NAME;
            jsonObj.start = localFormDateCal(
              response.data.data[i].APPOINT_TIME
            );
...
export const fetchEvents = (user_id, weekstart, weekend) => {
  return async (dispatch) => {
    await axios
      .get("https://teledoc.hicare.net:446/v1/doctor/appointment/schedules", {
        params: {
          user_id: user_id,
...
export const postSchedules = (userid, holiday, rperiod, events) => {
  return (dispatch) => {
    ...
    console.log(events);
    axios
      .post("https://teledoc.hicare.net:446/v1/doctor/appointment/schedules", {
        user_id: userid,
        holiday_yn: holiday,
        count: rperiod,
        events: dateToObj,
...
export const startschedules = (userid, weekstart, weekend, events) => {
  return (dispatch) => {
    axios
      .delete(
        "https://teledoc.hicare.net:446/v1/doctor/appointment/schedules",
        {
          data: {
            user_id: userid,
...
export const endchedules = (userid, weekstart, weekend, events) => {
  return (dispatch) => {
    ...
    axios
      .put("https://teledoc.hicare.net:446/v1/doctor/appointment/schedules", {
        user_id: userid,
        start_date: weekstart,
        end_date: weekend,
        events: dateToObj,
